            ...
            case LLM_ARCH_TALKIE:
                {
                    ...
                    // talkie has no learned final-norm scale — graph runs scaleless RMSNorm at output
                    output      = create_tensor(tn(LLM_TENSOR_OUTPUT, "weight"), {n_embd, n_vocab}, 0);
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        create_tensor_qkv(layer, i, n_embd, n_embd, n_embd_gqa, n_embd_gqa, 0);
                        layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd, n_embd}, 0);
                        // no attn_norm / attn_q_norm / attn_k_norm — talkie uses scaleless RMSNorm everywhere

                        layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd, n_ff}, 0);
                        layer.ffn_up   = create_tensor(tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd, n_ff}, 0);
                        layer.ffn_down = create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), {n_ff,  n_embd}, 0);

                        // per-layer embedding-skip scaling vector (tiled scalar
                        // across n_embd; works around a Metal mul-broadcast bug
                        // that NaNs for n_tokens >= 9 with a true [1] scalar)
                        layer.embed_skip_scale = create_tensor(tn(LLM_TENSOR_EMBED_SKIP, "weight", i), {n_embd}, 0);
                        // per-head Q gain (broadcast against [head_dim, n_head, n_tokens])
                        layer.attn_q_gain = create_tensor(tn(LLM_TENSOR_ATTN_Q_GAIN, "weight", i), {1, n_head}, 0);
                    }
                } break;
            case LLM_ARCH_SEED_OSS:
                {
